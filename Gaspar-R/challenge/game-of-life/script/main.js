...
const matrix = createMatrix(arrayWidth, arrayHeight);
initialArray = matrix;
...
function checkNeighbours(column, row, columnDesplacement, rowDisplacement, array) {
  const currentColumn = column + columnDesplacement;
  const currentRow = row + rowDisplacement;
...
function checkRepetedValues(resurrectionArray, currentColumn, currentRow) {
  const currentPosition = (element) => element[0] === currentColumn && element[1] === currentRow;
...
const array6 = searchAlives(newArray);
